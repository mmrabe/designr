...
design.contrasts <- function(design, factors = names(fixed.factors(design)), contrasts = NULL, expand = T, rename.contrasts = "%1$s%2$s", intercept = F, interactions = F, include.random.levels = F) {
  if(!is(design, "factor.design")) stop("`design` must be a factor design!")
  ...
  main.contrasts <- lapply(design[factors], function(fac) {
    if(is.null(fac)||!is.fixed.factor(fac)) stop(sprintf("At least one factor name given does not refer to a fixed factor", facname))
...
contrast.names <- function(design, ranfac = NULL, as.symbols = F, ...) {
  ...
  if(!is.null(ranfac)) {
    if(is.null(design[[ranfac]])||!is.random.factor(design[[ranfac]])) stop(sprintf("Random factor `%s` does not exist", ranfac))
