...
#'
#' This function simulates artificial data from a linear mixed-effects model. Parameters can either be set by hand using the parameters listed below *or* using a fitted \code{lmerMod} object.
#'
...
#' @param LMM if a \code{lmerMod} object containing a fitted \code{lmer} model is provided, then \code{simLMM} uses the estimated model parameters for data simulation. Note: If \code{LMM} is provided, \code{formula}, \code{Fixef}, \code{VC_sd}, \code{CP}, and \code{family} are taken from the provided fitted model, the arguments are ignored in the model call, and a warning is given.
...
#------------------------------------------------------------------------
  ...
  if(family == "lognormal") {
	...
	if (!is.null(LMM)) {
	  ...
	  if(!missing(formula)||!missing(family)||!missing(Fixef)||!missing(VC_sd)||!missing(CP)) warning('You can either provide the values for the parameters as formula, Fixef, VC_sd, CP, and family, *or* a fitted model as LMM (see ?simLMM). To avoid this message, please decide for one specification. Otherwise, if you provide LMM, other parameters are ignored.')
